...
router.get("/playlist/:playlistId/tracks", ensureValidAccessToken, async (req, res) => {
    try {
        const { playlistId } = req.params;
        const { offset = 0, limit = 20 } = req.query;
        const { data } = await axios.get(`https://api.spotify.com/v1/playlists/${playlistId}/tracks?offset=${offset}&limit=${limit}`, {
            headers: {
                Authorization: `Bearer ${req.session.accessToken}`,
            },
        });
...
router.put("/player", ensureValidAccessToken, async (req, res) => {
    try {
        ...
        const { data } = await axios.put("https://api.spotify.com/v1/me/player", { device_ids, play }, {
            headers: {
...
router.put("/player/play", ensureValidAccessToken, async (req, res) => {
    try {
        ...
        const { data } = await axios.put(`https://api.spotify.com/v1/me/player/play?device_id=${device_id}`, { uris, position_ms }, {
            headers: {
                "Content-Type": "application/json",
                Authorization: `Bearer ${req.session.accessToken}`,
            },
...
router.put("/player/seek", ensureValidAccessToken, async (req, res) => {
    try {
        ...
        if (position_ms == null)
            position_ms = req.query.position_ms;
        if (device_id == null)
        ...
        if (device_id)
            url.searchParams.set("device_id", String(device_id));
...
router.put("/player/pause", ensureValidAccessToken, async (req, res) => {
    try {
        ...
        const { data } = await axios.put(`https://api.spotify.com/v1/me/player/pause?device_id=${device_id}`, {}, {
            headers: {
                "Content-Type": "application/json",
                Authorization: `Bearer ${req.session.accessToken}`,
            },
