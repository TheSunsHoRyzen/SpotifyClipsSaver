...
// Refresh helper
const refreshAccessToken = async (req: any): Promise<void> => {
...
// Middleware
export const ensureValidAccessToken: express.RequestHandler = async (
  req,
  res,
  next
) => {
  try {
    ...
    const needsRefresh =
      !accessToken || invalidExpiry || now + 60_000 >= (expiresAt as number);
    ...
    console.error(
      "Failed to ensure valid access token:",
